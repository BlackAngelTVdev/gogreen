...
function getActiveEntriesInOrder(history, filterFn) {
...
export async function deleteHistoryEntry({ generationId }) {
  ...
  const activeEntriesForRepo = getActiveEntriesInOrder(state.history, (entry) => entry.repoUrl === generation.repoUrl && entry.kind === generation.kind);
